...
public class OAuth2ErrorController {
    ...
    public RedirectView loginError(@RequestParam(value = "error", required = false) String error) {
        if ("true".equals(error)) {
            ...
            return new RedirectView("http://localhost:3000/login?oauth_error=true"); // Default to port 3000
        }
        
        // Normal login redirect
        return new RedirectView("http://localhost:3000/login"); // Default to port 3000
    }
}